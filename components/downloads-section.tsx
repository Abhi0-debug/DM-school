...
import { DownloadItem, DocumentItem } from "@/lib/types";

function toDownloadItem(document: DocumentItem): DownloadItem {
  return {
    id: document.id,
  ...
  useEffect(() => {
    ...
    const loadDocuments = async () => {
      try {
        ...
        if (mounted && payload.documents) {
          setItems(payload.documents.map(toDownloadItem));
        }
      } catch {
    ...
    <section className="section-shell section-spacing">
      ...
      <div className="mt-8 space-y-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-700 dark:bg-slate-900">
        {items.map((item) => (
          ...
          >
            <div>
              <p className="break-words font-medium text-slate-900 dark:text-slate-100">{item.title}</p>
